...
def run_grounding_sam_mask(local_image_path, positive_prompt, negative_prompt, groundingdino_model, sam_predictor,
                      adjustment_factor):
    ...
    # Convert the final_mask to a PIL image for black and white mask
    final_mask_image = Image.fromarray(final_mask)

    # Return annotated image with color mask, and black and white mask
    return Image.fromarray(final_annotated_frame_with_mask), Image.fromarray(neg_annotated_frame_with_mask), final_mask_image, Image.fromarray(final_inverted_mask)
